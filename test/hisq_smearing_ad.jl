...
function hisq_smearing_ad_tests()
    ...
    directions = [
        LatticeMatrix(
            _hisq_smearing_ad_values(Val(NC), global_size, 41 + 11mu),
            4, process_grid; nw,
        ) for mu in 1:4
    ]
    left = [
        LatticeMatrix(
            _hisq_smearing_ad_values(Val(NC), global_size, 89 + 13mu),
            4, process_grid; nw,
        ) for mu in 1:4
    ]
    set_halo!.(U)
    ...
    @testset "HISQ level-1 Fat7 Enzyme pullback" begin
        ...
        for varied_direction in 1:4
            ...
            enzyme_directional = real(dot(
                dU[varied_direction], directions[varied_direction]))
            @test isapprox(
